...
const howMuchPencil = (str) => {
  ...
  for (let x = 0; x < toArray.length + 1; x++) {
    result.push(toArray.slice(x).join(""));
  }
  return result;
};
...
const listFoods = (recipe) => {
  ...
  for (let x = 0; x < recipe.ingredients.length; x++) {
    let ingredient = recipe.ingredients[x].slice(
      recipe.ingredients[x].indexOf(" ") + 1
    );
    let ingredient2 = ingredient.slice(ingredient.indexOf(" ") + 1);
